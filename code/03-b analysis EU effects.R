...
# Choose gas
gas <- "ch4"
data_temp <- read.csv(paste0("Data/02 intermediary/",gas,".csv"))

# Name sample (folder name)
sample_name <- "ch4_OECD_EUeffects"


# Add EU effects (dummys) -------------------------------------------------

data_full <- data_temp %>%
  filter(country %in% sample_countries) %>% 
  as_tibble()

effect_years <- c(1996:2022)

for(year_i in effect_years){
  
  effect_year <- paste0("EU", year_i)
  
  data_full <- data_full %>% 
    mutate(!!effect_year := ifelse(year>=year_i & country %in% EU15, 1, 0))
}

EU_dummies <- colnames(data_full)
EU_dummies <- EU_dummies[14:40]
...
x <- foreach(
  ...
  for(p.value in c(.05, .01, .001)){
    ...
    is <- isatpanel(
      data = dat,
      formula = paste0(
        "lemissions_pc ~ lgdp + lgdp_sq + lpop + ",
        paste(EU_dummies, collapse = " + ")
        ) %>% as.formula,
